...
def lstm_network(input_data, scope='lstm_network'):
    with tf.variable_scope(scope):
        ...
        _, states = tf.nn.dynamic_rnn(lstm_cells, input_data, dtype=tf.float32, initial_state=None)
...
def test(input_seq, num_itr):
    ...
    with tf.Session() as sess:
        ...
        for itr in range(num_itr):
            _ = sess.run([g_optimizer], feed_dict={g_encoder_input: cnn_batch_x, lstm_input: input_seq, bn_train: True})
            r_loss = sess.run([residual_loss], feed_dict={g_encoder_input: cnn_batch_x, lstm_input: input_seq, bn_train: True})
